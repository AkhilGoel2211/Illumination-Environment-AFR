...
def draw_polyboxes(frame, rec_dist, rec_class, bbs, ccs, dist_thresh, identified):
    for dist, label, bb, cc in zip(rec_dist, rec_class, bbs, ccs):
        ...
        if label not in identified:
            identified[label] = dist
        elif label in identified and identified[label] < dist:
            identified[label] = dist
            
    return frame


def video_recognize(image_classes, embeddings, det, face_model, identified, video_source=0, frames_per_detect=5, dist_thresh=0.6):
    ...
    while True:
        ...
        draw_polyboxes(frame, rec_dist, rec_class, bbs, ccs, dist_thresh, identified)
        cv2.imshow("", frame)
        cur_frame_idx += 1
    cap.release()
